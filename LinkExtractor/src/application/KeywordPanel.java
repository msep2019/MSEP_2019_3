...
public class KeywordPanel extends JPanel {
	...
	private void init() {
		 ...
		 * 
		 * JPanel panelKeywordLists = new JPanel(); panelKeywordLists.setLayout(new
		 * BoxLayout(panelKeywordLists, BoxLayout.X_AXIS));
		 * panelKeywordLists.setAlignmentX(Component.LEFT_ALIGNMENT);
		 * 
		 * JPanel panelEnabledKeywords = new JPanel();
		 * panelEnabledKeywords.setLayout(new BoxLayout(panelEnabledKeywords,
		 * BoxLayout.PAGE_AXIS));
		 * 
		 ...
		 * 
		 * DefaultListModel<String> modelEnabledKeywords = new DefaultListModel<>(); for
		 * (String item : enabledKeywords) { modelEnabledKeywords.addElement(item); }
		 * 
		 * listEnabledKeywords = new JList<>();
		 * listEnabledKeywords.setModel(modelEnabledKeywords);
		 * listEnabledKeywords.setPreferredSize(new Dimension(100, 200));
		 * listEnabledKeywords.setLayoutOrientation(JList.VERTICAL);
		 * listEnabledKeywords.setSelectionMode(ListSelectionModel.
		 * MULTIPLE_INTERVAL_SELECTION); JScrollPane paneEnabledKeywords = new
		 * JScrollPane(listEnabledKeywords);
		 ...
		 * panelDisabledKeywords.setLayout(new BoxLayout(panelDisabledKeywords,
		 * BoxLayout.Y_AXIS));
		 * 
		 ...
		 * 
		 * DefaultListModel<String> modelDisabledKeywords = new DefaultListModel<>();
		 * for (String item : enabledKeywords) { modelDisabledKeywords.addElement(item);
		 * }
		 * 
		 * listDisabledKeywords = new JList<>();
		 * listDisabledKeywords.setPreferredSize(new Dimension(100, 200));
		 * listDisabledKeywords.setModel(modelDisabledKeywords);
		 * listDisabledKeywords.setLayoutOrientation(JList.VERTICAL);
		 * listDisabledKeywords.setSelectionMode(ListSelectionModel.
		 * MULTIPLE_INTERVAL_SELECTION); JScrollPane paneDisabledKeywords = new
		 * JScrollPane(listDisabledKeywords);
		 * paneDisabledKeywords.setAlignmentX(Component.LEFT_ALIGNMENT);
		 * 
		 * panelDisabledKeywords.add(paneDisabledKeywords);
		 * 
		 * JPanel panelArrows = new JPanel(); panelArrows.setLayout(new
		 * BoxLayout(panelArrows, BoxLayout.Y_AXIS));
		 * 
		 * Icon icon1 = IconFontSwing.buildIcon(FontAwesome.ANGLE_LEFT, 20); Icon icon2
		 * = IconFontSwing.buildIcon(FontAwesome.ANGLE_RIGHT, 20); btnEnable = new
		 * JButton(icon1); btnDisable = new JButton(icon2);
		 * 
		 * 
		 * panelArrows.add(new JLabel(" ")); panelArrows.add(btnDisable);
		 * panelArrows.add(btnEnable);
		 * 
		 ...
		 * 
		 * JPanel panelAddKeyword = new JPanel(); panelAddKeyword.setLayout(new
		 * BoxLayout(panelAddKeyword, BoxLayout.X_AXIS));
		 * panelAddKeyword.setAlignmentX(Component.LEFT_ALIGNMENT);
		 * 
		 * txtKeyword = new JTextField(); panelAddKeyword.add(txtKeyword);
		 * 
		 * btnAdd = new JButton("Add"); panelAddKeyword.add(btnAdd);
		 * 
